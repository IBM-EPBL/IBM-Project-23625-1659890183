...
def signup():
    ...
    if request.method == "POST":
        ...
        if acnt:
            ...
            sg = sendgrid.SendGridAPIClient(api_key=os.environ.get('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'))
            from_email = Email("sushmasuresh945@gmail.com")
            to_email = To(email)
            subject = 'New SignUp'
            content = Content("text/plain","Hello, Your Registration was successfull. Thank you for choosing us.")
        ...
        mg = "fill out the form first!"
    return render_template("signup.html", msg=mg)
    ...
    if request.method == "POST":
        try:
            ...
            if dictionary:
                ...
                query = 'INSERT INTO orders (ID,STOCKS_ID,QUANTITY,DATE,DELIVER_DATE,PRICE) VALUES (?,?,?,?,?,?)'
                pstmt = ibm_db.prepare(conn, query)
                ibm_db.bind_param(pstmt, 1, id)
                ibm_db.bind_param(pstmt, 2, stock_id)
                ibm_db.bind_param(pstmt, 3, quantity)
                ibm_db.bind_param(pstmt, 4, date)
                ibm_db.bind_param(pstmt, 5, delivery_date)
                ibm_db.bind_param(pstmt, 6, price)
                ibm_db.execute(pstmt)
        except Exception as e:
            print(e)
...
def addSupplier():
    if request.method == "POST":
        try:  
            ...
            insert_sql = 'INSERT INTO suppliers (ID,NAME,ORDER_ID,LOCATION) VALUES (?,?,?,?)'
            pstmt = ibm_db.prepare(conn, insert_sql)
            ibm_db.bind_param(pstmt, 1, id)
            ibm_db.bind_param(pstmt, 2, name)
            ibm_db.bind_param(pstmt, 3, order_id)
            ibm_db.bind_param(pstmt, 4, location)
            ibm_db.execute(pstmt)
